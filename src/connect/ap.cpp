...
bool AP::resolve_ap(std::vector<std::pair<std::string,uint16_t>> &aps) {
    ...
    if (rc != CURLE_OK) {
        PLAT_LOGF("ap: apresolve failed (%d), using fallback", rc);
        return true;
    }
    PLAT_LOGF("ap: apresolve: %.400s", body.c_str());
...
bool AP::tcp_connect(const std::string &h, uint16_t p) {
    if (p == 80) {
        PLAT_LOGF("ap: skipping %s:80 (transparent proxy risk)", h.c_str());
        return false;
    }
    ...
    CURL *curl = curl_easy_init();
    if (!curl) { PLAT_LOG("ap: curl_easy_init failed"); return false; }

    curl_easy_setopt(curl, CURLOPT_URL,            url.c_str());
    curl_easy_setopt(curl, CURLOPT_CONNECT_ONLY,   1L);
    curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, 10L);

    PLAT_LOGF("ap: trying %s:%u via curl (raw TCP)", h.c_str(), p);
    CURLcode rc = curl_easy_perform(curl);
    if (rc != CURLE_OK) {
        PLAT_LOGF("ap: curl TCP to %s:%u failed: %s (%d)",
    ...
    PLAT_LOGF("ap: TCP connected to %s:%u (fd=%d TCP_NODELAY=%d)",
...
bool AP::send_raw(const uint8_t *buf, size_t len) {
    ...
    if (raw_recv_) {
        ...
        while (total_sent < len) {
            ...
            if (n < 0) {
                PLAT_LOGF("ap: raw send failed errno=%d total=%zu want=%zu",
    ...
    if (curl_) {
        ...
        while (len > 0) {
            ...
            if (rc != CURLE_OK || sent == 0) {
                PLAT_LOGF("ap: curl_easy_send failed rc=%d sent=%zu want=%zu total=%zu",
...
bool AP::recv_exact(uint8_t *buf, size_t n) {
    ...
    if (raw_recv_) {
        ...
        while (n > 0) {
            ...
            if (r > 0) {
                ...
                PLAT_LOG("ap: raw recv: connection closed");
                return false;
            } else if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
                plat_sleep_ms(500);
                if (++timeouts >= 360) { PLAT_LOG("ap: raw recv: 3 min timeout"); return false; }
            } else {
                PLAT_LOGF("ap: raw recv error errno=%d (want %zu)", errno, n);
                return false;
            }
        }
        return true;
    ...
    if (curl_) {
        ...
        while (n > 0) {
            ...
            if (rc != CURLE_AGAIN) {
                PLAT_LOGF("ap: curl_recv error rc=%d", (int)rc);
                return false;
            }
            ...
            if (pr < 0) {
                PLAT_LOGF("ap: poll error errno=%d", errno);
                return false;
            }
            if (pr == 0) {
                ...
                ++poll_timeouts;
                if (poll_timeouts >= 36) { PLAT_LOG("ap: recv_exact: timeout (3 min)"); return false; }
                continue;
            }

            if (pfd.revents & (POLLHUP | POLLERR)) {
                PLAT_LOG("ap: recv_exact: connection closed (HUP/ERR)");
                return false;
            }
...
bool AP::recv_plain(std::vector<uint8_t> &msg, std::vector<uint8_t> &raw) {
    ...
    uint8_t hdr[4];
    if (!recv_exact(hdr, 4)) {
        PLAT_LOG("ap: recv_plain: failed to read 4-byte header");
        return false;
    }
    uint32_t total = ((uint32_t)hdr[0]<<24)|((uint32_t)hdr[1]<<16)|
                     ((uint32_t)hdr[2]<<8)|(uint32_t)hdr[3];
    if (total < 4 || total > 65536) {
        PLAT_LOGF("ap: recv_plain: bad total=%u (hdr=%02X%02X%02X%02X)",
                     total, hdr[0], hdr[1], hdr[2], hdr[3]);
        return false;
    }
    msg.resize(total - 4);
    if (!msg.empty() && !recv_exact(msg.data(), msg.size())) {
        PLAT_LOG("ap: recv_plain: failed to read proto body");
        return false;
    }
...
bool AP::do_handshake() {
    // ── DH key pair ──────────────────────────────────────────────────────────
    PLAT_LOG("ap: handshake: generating DH key...");
    ...
    PLAT_LOGF("ap: handshake: DH key ready (ret=%d, len=%zu, pub[0..3]=%02X%02X%02X%02X)",
                 mpi_ret, our_pub_len,
                 our_pub[0], our_pub[1], our_pub[2], our_pub[3]);
    if (mpi_ret != 0) {
        PLAT_LOGF("ap: handshake: DH keygen failed (-0x%04X)", -mpi_ret);
    ...
    PLAT_LOGF("ap: handshake: sending ClientHello (%zu bytes)...",
                 client_hello_raw.size());
    if (!send_raw(client_hello_raw.data(), client_hello_raw.size())) {
        PLAT_LOG("ap: handshake: send ClientHello failed");
        mbedtls_mpi_free(&P_mpi); mbedtls_mpi_free(&X_mpi);
        return false;
    }
    PLAT_LOG("ap: handshake: ClientHello sent, waiting for APResponse...");

    // ── Receive APResponseMessage ─────────────────────────────────────────────
    std::vector<uint8_t> ap_msg, ap_raw;
    if (!recv_plain(ap_msg, ap_raw)) {
        mbedtls_mpi_free(&P_mpi); mbedtls_mpi_free(&X_mpi);
        PLAT_LOG("ap: recv APResponseMessage failed");
        return false;
    }
    ...
    while (root.next(f, w)) {
        if (f == 10 && w == 2) {  // APChallenge
            ...
            PLAT_LOG("ap: APResponse: UpgradeRequired -- old client rejected");
            ...
            PLAT_LOGF("ap: APResponse: APLoginFailed error_code=%llu", (unsigned long long)err);
        } else {
            PLAT_LOGF("ap: APResponse: unknown field %u wire %u", f, w);
            root.skip(w);
        }
    }
    ...
    if (server_pub.empty() || server_nonce.size() != 16) {
        mbedtls_mpi_free(&P_mpi); mbedtls_mpi_free(&X_mpi);
        PLAT_LOG("ap: bad APHello -- missing server pubkey or nonce");
        return false;
    }
    // ── DH shared secret ──────────────────────────────────────────────────────
    ...
    if (!send_plain(hello_resp)) {
        PLAT_LOG("ap: send ClientHelloResponse failed");
        return false;
    }
    ...
    PLAT_LOG("ap: switched to raw recv");
    raw_recv_ = true;

    PLAT_LOG("ap: handshake complete");
    return true;
}
...
bool AP::do_login(const Discovery::Credentials &creds) {
    PLAT_LOGF("ap: login: user='%s' auth_type=%u auth_data_len=%zu",
    ...
    pb_str(sys_info, 90, "Switch;1.1.0;en");
    ...
    PLAT_LOGF("ap: login: sending Login cmd=0xAB payload_len=%zu", cre.size());
    if (!send_packet(Cmd::Login, cre)) {
        PLAT_LOG("ap: login: send Login packet failed");
        return false;
    }
    PLAT_LOG("ap: login: Login packet sent, waiting for response...");
    ...
    uint8_t hdr[3];
    if (!recv_exact(hdr, 3)) {
        PLAT_LOG("ap: recv login response header failed");
        return false;
    }
    ...
    PLAT_LOGF("ap: login response: cmd=0x%02X plen=%zu", cmd, plen);
    ...
    if (memcmp(mac_buf, expected_mac, 4) != 0) {
        PLAT_LOG("ap: login response MAC mismatch");
        return false;
    }
    ...
    if (cmd == Cmd::APWelcome) {
        ...
        PLAT_LOGF("ap: logged in as '%s'", username_.c_str());
        ...
        if (reusable_creds_.empty()) {
            FILE *fc = fopen("sdmc:/spotify_saved_creds.bin", "rb");
            if (fc) {
        ...
        if (!reusable_data.empty()) {
            PLAT_LOGF("ap: reusable creds type=%u len=%zu", reusable_type, reusable_data.size());
            ...
            FILE *f2 = fopen("sdmc:/spotify_saved_creds.bin", "rb");
            if (!f2) {
                f2 = fopen("sdmc:/spotify_saved_creds.bin", "wb");
                if (f2) {
                    uint8_t tb = (uint8_t)reusable_type;
                    fwrite(&tb, 1, 1, f2);
                    fwrite(reusable_data.data(), 1, reusable_data.size(), f2);
                    fclose(f2);
                    PLAT_LOG("ap: saved creds written to SD card");
                }
            } else {
                fclose(f2);
                PLAT_LOG("ap: creds already on SD card, not overwriting");
            }
        }
        return true;
    }
    ...
    if (cmd == Cmd::AuthFailure) {
        ...
        PLAT_LOGF("ap: auth failure, error_code=%u", err_code);
        return false;
    }

    PLAT_LOGF("ap: unexpected login response cmd=0x%02X", cmd);
    return false;
}
...
bool AP::connect(const Discovery::Credentials &creds, Callbacks cb) {
    ...
    for (auto &[h, p] : aps) {
        ...
        if (!do_handshake()) {
            PLAT_LOGF("ap: handshake failed on %s:%u, trying next", h.c_str(), p);
        ...
        if (!do_login(creds)) {
            PLAT_LOGF("ap: login failed on %s:%u, trying next", h.c_str(), p);
            if (curl_) { curl_easy_cleanup(curl_); curl_ = nullptr; } fd_ = -1; raw_recv_ = false; continue;
        }
        connected_.store(true);
        stop_.store(false);
        { pthread_attr_t a; pthread_attr_init(&a); pthread_attr_setstacksize(&a, 1024*1024);
          pthread_create(&recv_thread_, &a, [](void *v) -> void* { static_cast<AP*>(v)->recv_loop(); return nullptr; }, this);
          pthread_attr_destroy(&a); }
        return true;
    }
    if (!stop_.load())
        PLAT_LOG("ap: all AP candidates exhausted");
    return false;
}
...
void AP::disconnect() {
    ...
    if (recv_thread_) { pthread_join(recv_thread_, nullptr); recv_thread_ = 0; }
    if (curl_) {
...
bool AP::send_packet(uint8_t cmd, const uint8_t *payload, size_t len) {
    if (!send_cipher_) {
        PLAT_LOGF("ap: send_packet 0x%02X: no cipher!", cmd);
        return false;
    }
    ...
    if (!send_raw(wire.data(), wire.size())) {
        PLAT_LOGF("ap: send_packet 0x%02X: send failed", cmd);
        return false;
    }
    return true;
}
...
void AP::recv_loop() {
    ...
    while (!stop_.load()) {
        ...
        if (memcmp(mac_buf, expected, 4) != 0) {
            PLAT_LOG("ap: MAC verification failed -- closing");
            break;
        }
...

        {
            ...
            PLAT_LOGF("ap: recv cmd=0x%02X len=%zu: %s", cmd, payload.size(), hex);
        }
